...
public class AvailabilityController {
    ...
    @GetMapping
    @Operation(summary = "Search eligible rooms for a stay", description = "Anonymous access. Uses [checkIn,checkOut), capacity, catalog activity and operational status. Prices are nightly in hotel currency (PEN by default). Excludes overlapping CONFIRMED, CHECKED_IN and CHECKED_OUT reservations. CANCELLED and NO_SHOW do not block. Results remain indicative; creation revalidates against PostgreSQL. This read never holds or reserves a room.")
