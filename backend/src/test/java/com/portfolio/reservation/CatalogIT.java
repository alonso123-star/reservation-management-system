...
class CatalogIT {
    ...
    @BeforeEach void prepare() {
        jdbc.execute("TRUNCATE rooms, room_types, auth_rate_limits, audit_events, refresh_tokens, refresh_sessions, users");
    ...
    @Test void migrationAndOpenApiContainOnlyTheCurrentCatalogScope() throws Exception {
        ...
        assertThat(jdbc.queryForList("SELECT tablename FROM pg_tables WHERE schemaname='public'", String.class))
                .contains("room_types", "rooms").doesNotContain("reservations", "payments", "idempotency_requests");
